...
def analyze_channel(
    ...
    if skip_langdetect:
        config.set("processing.skip_langdetect", True)
    ...
    # Save config snapshot for reproducibility
    config.save_snapshot(output_path)
    ...
    data_loader = DataLoader(max_text_length=config.max_text_length)
    df = data_loader.load_data(
        input_path=input_path,
        format_type=config.input_format,
    ...
    ner_processor = NERProcessor(
        ...
        max_entities_per_msg=config.max_entities_per_msg,
        device=device,
    )
